...
Under the hood, `marginaleffects`'s `predictions` function uses the `insight` package to retrieve adjusted predictions from a wide variety of models. Currently, `insight` [supports over 100 model types](https://easystats.github.io/insight/), and many should work out-of-the-box with the `predictions` function. If you run into problems, do not hesitate to report them on Github or via email.
