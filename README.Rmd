...
and obtain the following estimates: $\beta_1=1; \beta_2=2$. By taking the partial derivative with respect to $X$ we obtain the marginal effect of $X$ on $Y$ variable:

$$\partial Y / \partial X = \beta_1 + 2 \cdot \beta_2 X$$

Plugging in our estimates leads us to conclude that the effect of $X$ on $Y$ increases as $X$ itself increases. An increase in $X$ is associated with $1 + 4X$. When $X$ is large, an increase in $X$ has a large effect on $Y$. When $X$ is small, an increase in $X$ has a small effect on $Y$.
