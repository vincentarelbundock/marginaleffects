...
`marginaleffects` offers a series of "model-agnostic" to interpret statistical and machine learning models. This vignette highlights how the package can be used to interpret the results of models trained using the `mlr3` and `tidymodels` framework.
...
Make sure to restart `R` after installation.
...
As the code above makes clear, the `avg_comparisons()` computes the effect of a "centered" change on the outcome. If we want to compute a "Forward Marginal Effects" instead, we can call:
