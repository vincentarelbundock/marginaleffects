get_by <- function(
  ...
  # useful for named hypothesis strings in avg_predictions()
  if (!"term" %in% colnames(estimates)) {
    te <- estimates[, ..bycols, drop = FALSE]
    te <- apply(te, 1, paste, collapse = " ")
    estimates$term <- te
    bycols <- c("term", bycols)
  }
