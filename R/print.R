...
print.marginaleffects <- function(x,
  ...
  # in predictions() the term is just pasted by=
  # convenient for hypothesis testing, but redundant in printing
  if (inherits(x, "predictions") && is.null(attr(x, "call")$hypothesis)) {
    dict <- dict[names(dict) != "term"]
  }
