...
The fitted model estimates a hazard ratio of about 1.34 for hormonal treatment. Conditional on `age` and `grade`, this corresponds to an approximately 34% higher instantaneous hazard of death under the proportional-hazards model. We will investigate this further below.
...
The function we use to implement these steps is `avg_predictions()`. We set `type="survival"` to obtain the average counterfactual predictions on the survival probability scale. For Cox models, `type="risk"` returns the relative risk score $\exp(x^\top\hat\beta)$, not a death probability; the probability of death by `dtime` is $1-S(\texttt{dtime})$.
...
The `vcov` argument is set to `"rsample"` to compute percentile bootstrap confidence intervals with the `rsample` package. The intervals and associated inferences below are pointwise; they do not provide simultaneous coverage over an entire curve.

Because we want to calculate counterfactual predictions, we also had to define the `newdata` argument using a call to the `datagrid()` function. As usual, this argument controls over which values of the predictors the predictions should be made. Since there are many `dtime` values, replicating the full dataset would be very memory- and computationally-intensive. Therefore, we use a grid of 25 equally spaced points from day 0 to a prediction horizon of day 4000, when 414 patients remain at risk, and feed it to `avg_predictions()`.
...
The resulting plot shows the survival curves for both treatment options of `hormon`, adjusted for `age` and `grade`. Unlike a single hazard ratio, it displays how adjusted survival evolves over time. Group comparisons should be based on direct contrasts rather than overlap between separate intervals.

### Hypothesis Testing

To compare survival probabilities between the two `hormon` groups at selected times, we estimate their difference directly [@Klein2007]. In other words, one may wish to check if we can reject this null hypothesis:

> At time $t$, the difference between the estimated survival probability for patients in group `hormon=1` and `hormon=0` is equal to zero. 

This kind of null hypothesis can be tested easily with the `avg_predictions()` function. In the code below, we specify a new grid with three different values of `dtime`: 1000, 2500, and 4000. Then, we use [the convenient formula syntax](https://marginaleffects.com/chapters/hypothesis.html) of the `hypothesis` argument to compute differences in estimated survival probabilities for each `dtime`.
...
The `Estimate` column of the output now includes the estimated difference between counterfactual survival probabilities at $t\in\{1000,2500,4000\}$. At each selected time, the difference is negative and its pointwise confidence interval excludes zero, providing pointwise evidence against equal survival probabilities.
...
Here, the `Estimate` argument shows the ratio of the two survival probabilities at $t$ instead of the difference, again with an associated 95% pointwise confidence interval. Each interval excludes 1, providing pointwise evidence against equal survival probabilities at each selected time.
...
In the real world, we often encounter heterogeneous treatment effects, meaning that the causal effect of one variable on an outcome is not necessarily the same in different groups of individuals. For example, we might be interested in assessing whether the effect of the hormonal treatment differs by values of `grade`. The classic approach to assess such heterogeneity is to allow a multiplicative interactions in the Cox model formula:
...
b. Row 11: `hormon=1` and `grade=2` and `dtime=4000`
...
Instead of one adjusted survival curve per `hormon` group, we now get four adjusted survival curves in total. The curves suggest greater separation for `grade = 3` than for `grade = 2`, but whether the treatment contrast differs by grade should be assessed directly, as below.
...
To start, we ask: What is the estimated average treatment effect of `hormon` on the survival probability of people in our sample, at time $t=4000$.
...
On average, moving from `hormon = 0` to `hormon = 1` is associated with a `r round(cmp$estimate[1] * 100)` percentage point change in the expected survival probability at $t=4000$.
...
For individuals in `grade = 2`, moving from `hormon = 0` to `hormon = 1` is associated with a `r round(cmp$estimate[1] * 100)` percentage point change in the expected survival probability at $t=4000$. For individuals in `grade = 3`, the corresponding change is `r round(cmp$estimate[2] * 100)` percentage points.
...
The pointwise confidence interval for the difference between the grade-specific survival-probability contrasts includes zero. At $t=4000$, this provides insufficient evidence that the survival-probability difference associated with `hormon` varies by grade.
...
Before concluding, it is useful to briefly discuss the challenge of estimating uncertainty around marginal estimates in the types of models considered here. The Cox proportional-hazards model separates relative covariate effects from an unspecified baseline hazard:

$$
h(t\mid x)=h_{0}(t)\,\exp\{x^{\top}\beta\},
$$

where $h_{0}(t)$ is the baseline hazard and $\beta$ is a vector of regression coefficients.

Under the proportional-hazards model and the usual independent-censoring assumptions, the regression coefficients $\beta$ can be estimated by maximizing the partial likelihood. At each event time, this likelihood compares the covariates of the individuals who experience an event with those of individuals still at risk, thereby eliminating $h_{0}(t)$ from the likelihood used to estimate $\beta$.

Absolute predictions require an additional step. After estimating $\beta$, we estimate the cumulative baseline hazard $H_{0}(t)$, commonly with the Breslow estimator. Because $\hat H_{0}(t)$ depends on $\hat\beta$ and on the same event and risk-set data, the two estimates are statistically dependent. Consequently, linear-predictor and relative-risk quantities depend only on $\hat\beta$, whereas absolute quantities such as cumulative hazards and survival probabilities depend jointly on $\hat\beta$ and $\hat H_{0}(t)$.

For example, a prediction on the linear-predictor scale is
...
The variance of this prediction can be obtained from the estimated variance--covariance matrix of the coefficients alone. By contrast, uncertainty for survival and cumulative-hazard predictions must also account for the estimation of $H_{0}(t)$ and its dependence on $\hat\beta$.

`marginaleffects` applies the multivariate delta method to propagate uncertainty from $\hat\beta$ to any user-defined function $g(\hat\beta)$. For Cox models, the current implementation does not propagate all uncertainty in $\hat H_{0}(t)$. This can make confidence intervals for absolute survival or cumulative-hazard predictions too narrow, sometimes substantially. The effect can be much smaller for contrasts because shared baseline-hazard uncertainty may cancel.
