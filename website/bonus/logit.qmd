...
Unlike ORs, adjusted risk differences vary from individual to individual based on the values of the control variables. The `comparisons()` function can compute adjusted risk differences for every individual. Here, we display only a few of them:
...
Population-averaged (aka "marginal") adjusted risk difference ([see this vignette](/chapters/gcomputation.html)) can be obtained using the `avg_*()` functions or using the `by` argument:
...
We can present the entire distribution of unit-level proportion differences as a cumulative distribution function:
