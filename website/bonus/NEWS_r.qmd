...
* The `autodiff()` function and the JAX-backed automatic differentiation path have been removed, along with the `reticulate` dependency. Exact analytic Jacobians now supersede them: they cover strictly more model classes and more built-in `comparison` functions than the JAX pipeline ever did, they are faster, they require no Python installation, and they are on by default with no opt-in. Calls to `autodiff()` will error; delete them. Standard errors previously produced with `autodiff(TRUE)` are unchanged up to floating-point noise, because both paths are exact.

Performance:

* Built-in comparison functions other than the differences -- `ratio`, `lnratio`, `lnor`, `lift`, and the slope and elasticity shortcuts -- now compose exact prediction Jacobians with hand-derived closed-form derivatives of the comparison stage, instead of differentiating the whole pipeline once per coefficient. About 5x faster for `avg_comparisons(comparison = "ratio")` on a model with 121 coefficients, and the gap grows with the number of coefficients. User-supplied `comparison` functions keep the numeric path: arbitrary code has no recorded closed form, and probing it cannot prove one.
* `hypothesis` strings and formula shortcuts which are linear in the estimates are now compiled to exact contrast matrices, which the analytic Jacobian consumes. Issue #1735. Linearity is established by walking the parsed syntax tree, never by numeric probing, and the same walk reads the matrix coefficients directly off the tree. User-supplied hypothesis functions and arbitrary formula code are not promoted, because no finite set of evaluations can prove linearity of arbitrary code; their standard errors compose through a derivative taken at the estimate instead.
...
* A custom `comparison` function which does not use the observed outcome no longer disqualifies the stage-composed derivative of a downstream `hypothesis`: the hypothesis stage is differentiated on its own and composed with the numeric model derivative. The comparison stage itself keeps the numeric path either way -- arbitrary code has no recorded closed form -- so `jacobian_method` still reports `"numeric"`.
* New `jacobian_method` component records which differentiation path produced the Jacobian: `components(x, "jacobian_method")` returns `"analytic"`, `"numeric"`, or `"custom"`. The label reflects what actually ran: a custom Jacobian function which returns `NULL` falls back to numerical differentiation and is labeled `"numeric"`.
* A matrix returned by the `marginaleffects_jacobian_function` setting is now honored even when a `hypothesis` is present, as its documentation promised, and its dimensions are validated against the number of estimates instead of being silently recycled. Hook authors: the function receives the `hypothesis` argument and must return the Jacobian of the *final* estimates. A hook which ignores `hypothesis` and returns a pre-hypothesis Jacobian was previously overwritten by numerical differentiation; it is now trusted, and when the two shapes coincide no dimension check can catch the mistake.
...
* Weighted average comparisons (`avg_comparisons(wts = ...)` and friends) returned `NaN` estimates and standard errors when the weight of a group's first row was exactly zero. The weighted comparison functions had already consumed the weights and collapsed each group to one row, but a redundant second aggregation re-weighted that lone row by its stale unit-level weight, dividing zero by zero. Zero weights are routine in ATT and matching workflows, and the failure depended on row order. The collapsed row now carries its group's *total* weight into any further aggregation -- the pooled weighted mean -- on the frequentist, Bayesian, and analytic-Jacobian paths alike. A genuine group whose entire weight is zero remains `NaN`, because that weighted mean is undefined.
* An affine `hypothesis` -- linear in the estimates plus a constant, such as `"b1 = 5"` or `"b1 + 1e8 = 0"` -- is now compiled to an exact contrast matrix with an offset, and its derivative is the matrix alone. Previously the constant was differentiated numerically along with the rest of the map, and a constant large relative to the estimates cancelled the probe step catastrophically: `"b1 + 1e16 = 0"` reported a standard error of exactly zero with full confidence.
* `hypotheses()` applied to an already-computed object -- a `predictions()`, `comparisons()`, or `slopes()` result, a model, or a data frame of estimates -- now takes the exact derivative of an affine string hypothesis instead of finite-differencing the whole hypothesis map. A constant large relative to the estimates cancelled the probe step catastrophically, so `hypotheses(p, hypothesis = "b2 - b1 - 1e10 = 0")` reported a standard error of exactly zero while `"b2 - b1 = 0"` reported the correct value. The conditional pipeline was already fixed on this development version; this is the post-hoc path. Found in internal review.
* `vcov = "unconditional"` now uses the same exact hypothesis derivative as the other standard error paths. Both unconditional components -- the coefficient Jacobian and the empirical influence function -- differentiated the hypothesis map numerically, so an affine hypothesis with a constant large relative to the estimates, such as `"b1 - 1e10 = 0"`, cancelled the probe step and reported a standard error of zero or `NA`. Found in internal review.
* The `~meandev` and `~meanotherdev` hypothesis shortcuts no longer materialize their operators as `n x n` matrices, which are dense in every entry (573 MB at ten thousand estimates). The centering is applied to estimates and to Jacobian rows directly, in `O(np)` time and memory, and remains exact.
* Comparison rows are dropped when *either* prediction is missing, not only the low one. A finite low prediction paired with a missing high one previously survived into the comparison stage and surfaced as a misleading complaint about the `comparison` function.
...
* A standard error of exactly zero is now reported as `0` instead of `NA`. A constant estimand -- a contrast of a level with itself, for example -- has variance exactly zero; its test statistic is undefined and comes out `NaN`. This also matches the Python numerical path. Note that on the numerical differentiation path a zero can also signal a finite-difference step which underflowed; the previous `NA` blurred the two cases together, and a suspicious zero is best cross-checked with `numderiv = "richardson"`.
...
* `hypotheses()` no longer errors with `survey::svyglm()` objects when no averaging is required. Thanks to @bdwilliamson for Issue #1723.
