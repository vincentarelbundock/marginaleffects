...
# Seeded so regenerating the fixtures is reproducible: `Char` is not in the
# fitted model, but it is passed through into the output CSVs.
set.seed(1024)
...
comparisons(mod, comparison = "differenceavg") |>
    fwrite(here("tests/r/test_comparisons_01.csv"))
comparisons(mod, comparison = "difference", eps = 1e-4) |>
    fwrite(here("tests/r/test_comparisons_02.csv"))
...
for (e in estimands) {
    comparisons(mod, comparison = e, newdata = Guerry) |>
        fwrite(here(paste0("tests/r/test_comparisons_03_", e, ".csv")))
}


# test_by()
comparisons(mod, comparison = "differenceavg", by = "Region") |>
    fwrite(here("tests/r/test_comparisons_04.csv"))


# test_HC3()
comparisons(mod, comparison = "differenceavg", vcov = "HC3") |>
    fwrite(here("tests/r/test_comparisons_05.csv"))


# test_difference_wts()
comparisons(mod, variables = "Desertion", by = "Region", wts = "Literacy") |>
    fwrite(here("tests/r/test_comparisons_06.csv"))
comparisons(mod, variables = "Desertion", by = "Region") |>
    fwrite(here("tests/r/test_comparisons_07.csv"))
