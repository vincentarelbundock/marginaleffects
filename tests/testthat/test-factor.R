...
if (requireNamespace("estimatr", quietly = TRUE)) {
    model <- estimatr::lm_robust(carb ~ wt + factor(cyl), se_type = "stata", data = mtcars)
    k <- slopes(model)
...
if (requireNamespace("survival", quietly = TRUE)) {
    ...
    mod <- survival::coxph(survival::Surv(time, status) ~ x + survival::strata(sex), data = test1, ties = "breslow")
