...
expect_equal(contr2$estimate, rep(iqr, 32), tolerance = 1e-6, ignore_attr = TRUE)
expect_equal(contr3$estimate, rep(minmax, 32), tolerance = 1e-6, ignore_attr = TRUE)
expect_equal(contr4$estimate, rep(sd1, 32), tolerance = 1e-6, ignore_attr = TRUE)
expect_equal(contr5$estimate, rep(sd2, 32), tolerance = 1e-6, ignore_attr = TRUE)
...
expect_equal(contr$estimate[1], pred$estimate[pred$cyl == 6] - pred$estimate[pred$cyl == 4], tolerance = 1e-6, ignore_attr = TRUE)
...
if (requireNamespace("emmeans", quietly = TRUE)) {
    ...
    emm <- emmeans::emmeans(mod, specs = "cyl")
    ...
    expect_equal(cmp$std.error, emm$SE, tolerance = 1e-5, ignore_attr = TRUE)
    expect_equal(cmp$conf.low, emm$asymp.LCL, tolerance = 1e-5, ignore_attr = TRUE)
    expect_equal(cmp$conf.high, emm$asymp.UCL, tolerance = 1e-5, ignore_attr = TRUE)
