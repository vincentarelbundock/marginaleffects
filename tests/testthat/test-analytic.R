...
expect_equal(res$estimate, res$truth, tolerance = .01, ignore_attr = TRUE)
...
expect_equal(as.numeric(marg$estimate[marg$term == "x"]), dydx, tolerance = tol, label = "dy/dx correct", ignore_attr = TRUE)
expect_equal(
    sedydx,
    as.numeric(marg$std.error[marg$term == "x"]),
    tolerance = tol_se,
    label = "Var(dy/dx) correct",
    ignore_attr = TRUE
)
# ME with respect to z
dydz <- coef(m)["z"] + (d$x * coef(m)["x:z"])
sedydz <- sqrt(vcov(m)["z", "z"] + (d$x^2 * vcov(m)["x:z", "x:z"]) + (2 * d$x * vcov(m)["z", "x:z"]))
expect_equal(as.numeric(marg$estimate[marg$term == "z"]), dydz, tolerance = tol, label = "dy/dz correct", ignore_attr = TRUE)
expect_equal(
    sedydz,
    as.numeric(marg$std.error[marg$term == "z"]),
    tolerance = tol_se,
    label = "Var(dy/dz) correct",
    ignore_attr = TRUE
)
