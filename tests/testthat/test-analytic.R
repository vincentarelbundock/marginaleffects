test_that("lm quadratic", {
    ...
    mod <- lm(f, dat)
    nd <- typical(newdata = dat, x = c(-2:2))
...
test_that("lm log", {
    ...
    mod <- lm(f, dat)
    nd <- typical(newdata = dat, x = c(1:4))
    res <- marginaleffects(mod, newdata = nd)
    res$truth <- truth(res$x)
    expect_equal(res$dydx, res$truth, tolerance = .01)
})
...
test_that("logit", {
    ...
    nd <- typical(newdata = dat, x = c(-10:10))
    res <- marginaleffects(mod, newdata = nd)
    res$truth <- truth(res$x)
    expect_equal(res$dydx, res$truth, tolerance = .01)
})
...
test_that("Golder Interaction Case 1a/1b correct", {
    f1.1 <- y ~ x + z + x:z
    m <- lm(f1.1, data = d)
    marg <- marginaleffects(m)
    # ME with respect to x
    dydx <- coef(m)["x"] + (d$z * coef(m)["x:z"])
    sedydx <- sqrt(vcov(m)["x","x"] + (d$z^2 * vcov(m)["x:z","x:z"]) + (2 * d$z * vcov(m)["x","x:z"]))
    expect_equal(as.numeric(marg$dydx[marg$term == "x"]), dydx, tolerance = tol, label = "dy/dx correct")
    expect_equal(sedydx, as.numeric(marg$std.error[marg$term == "x"]), tolerance = tol_se, label = "Var(dy/dx) correct")
    # ME with respect to z
    dydz <- coef(m)["z"] + (d$x * coef(m)["x:z"])
    sedydz <- sqrt(vcov(m)["z","z"] + (d$x^2 * vcov(m)["x:z","x:z"]) + (2 * d$x * vcov(m)["z","x:z"]))
    expect_equal(as.numeric(marg$dydx[marg$term == "z"]), dydz, tolerance = tol, label = "dy/dz correct")
    expect_equal(sedydz, as.numeric(marg$std.error[marg$term == "z"]), tolerance = tol_se, label = "Var(dy/dz) correct")
})
