...
expect_error(slopes(tmp, hypothesis = "drat = wt"), regexp = "newdata")
expect_error(comparisons(tmp, hypothesis = "drat = wt"), regexp = "newdata")
...
expect_equal(nrow(mfx), 1, ignore_attr = TRUE)
...
expect_equal(diff(cmp1$estimate), cmp2$estimate[1], tolerance = 1e-6, ignore_attr = TRUE)
...
expect_equal(p1$estimate, diff(p2$estimate), tolerance = 1e-6, ignore_attr = TRUE)
...
expect_equal(sum(p1$estimate) - 10, p2$estimate, tolerance = 1e-6, ignore_attr = TRUE)
expect_equal(p1$estimate[1] - p1$estimate[2], p3$estimate, tolerance = 1e-6, ignore_attr = TRUE)
...
expect_equal(dm$term, c("H1", "H2"), ignore_attr = TRUE)
...
expect_equal(p2$estimate, p1$estimate - mean(p1$estimate), tolerance = 1e-6, ignore_attr = TRUE)
p2 <- avg_predictions(mod, by = "cyl", hypothesis = ~meanotherdev)
expect_equal(p2$estimate, p1$estimate - (sum(p1$estimate) - p1$estimate) / (nrow(p1) - 1), tolerance = 1e-6, ignore_attr = TRUE)
...
requiet("brms")
requiet("rstan")
brms_factor_mod <- readRDS(testing_path("modelarchive/data/brms_factor.rds"))
p <- avg_predictions(brms_factor_mod, by = "cyl_fac", hypothesis = ~reference)
d <- get_draws(p)
expect_s3_class(p, "predictions")
expect_true("hypothesis" %in% colnames(p))
expect_true(all(c("(6) - (4)", "(8) - (4)") %in% d$hypothesis))

p <- predictions(brms_factor_mod, hypothesis = ~ I(c(a = x[1], b = mean(x[1:2]))) | cyl_fac)
d <- get_draws(p)
expect_s3_class(p, "predictions")
expect_true("hypothesis" %in% colnames(p))
...
p <- predictions(brms_factor_mod, hypothesis = ~ I(mean(x)) | cyl_fac)
expect_s3_class(p, "predictions")
expect_false("hypothesis" %in% colnames(p))
...
expect_equal(h$estimate, c(1.350545980792, 1.76580373269115), ignore_attr = TRUE)
h <- hypotheses(mod, hypothesis = ~revreference)
expect_true("(cyl4) - (cyl6)" %in% h$hypothesis)
expect_equal(h$estimate, c(6.92077922077922, 11.5636363636364), ignore_attr = TRUE)
cmp1 <- avg_comparisons(mod, hypothesis = ~revreference)
cmp2 <- avg_comparisons(mod, hypothesis = ~reference)
expect_equal(cmp1$estimate, cmp2$estimate * -1, ignore_attr = TRUE)
...
expect_equal(p1$p.value + p2$p.value, 1, ignore_attr = TRUE)

p1 <- avg_predictions(mod, by = "cyl", hypothesis = "b1 = 26")
p2 <- avg_predictions(mod, by = "cyl", hypothesis = "b1 >= 26")
expect_equal(p1$p.value, p2$p.value * 2, ignore_attr = TRUE)
...
expect_equal(p1, p2, tolerance = 1e-5, ignore_attr = TRUE)
expect_equal(p1, p3, tolerance = 1e-5, ignore_attr = TRUE)
...
expect_equal(h1$p.value, h2$test$pvalues, tolerance = 1e-5, ignore_attr = TRUE)
...
h2 <- glht(mod, linfct = c("(Intercept) <= -3.5", "hp <= -3.5", "wt <= -3.5"))
h2 <- summary(h2, test = univariate())
expect_equal(h1$p.value, h2$test$pvalues, ignore_attr = TRUE)

h1 <- hypotheses(mod, hypothesis = c("hp = -3.5", "wt = -3.5"), df = 29)
h2 <- glht(mod, linfct = c("hp == -3.5", "wt == -3.5"))
h2 <- summary(h2, test = univariate())
expect_equal(h1$p.value, h2$test$pvalues, tolerance = 1e-5, ignore_attr = TRUE)
expect_equal(coef(mod)[2:3] + 3.5, h1$estimate, ignore_attr = TRUE)
...
expect_equal(h1$p.value, c(0.000243703399238325, 0.191786862518089), ignore_attr = TRUE)
expect_equal(h2$p.value, c(0.000243703399238325, 0.191786862518089), ignore_attr = TRUE)
