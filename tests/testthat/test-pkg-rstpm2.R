test_that("rstpm2 package works", {
    skip_if_not_installed("rstpm2")

    withr_library("rstpm2")
    ...
    expect_equal(pred1, pred2$estimate, ignore_attr = TRUE)

    pred3 <- predict(test1, newdata = nd, type = "meansurv")
    pred4 <- avg_predictions(test1, type = "surv", newdata = nd, by = "x3")
    expect_equal(pred3[1], pred4$estimate, ignore_attr = TRUE)
})
