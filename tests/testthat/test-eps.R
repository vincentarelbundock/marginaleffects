...
m2 <- slopes(mod, eps = 1)
m3 <- slopes(mod, eps = 1e-4)
expect_equal(m1$estimate, m2$estimate, ignore_attr = TRUE)
expect_equal(m1$estimate, m3$estimate, ignore_attr = TRUE)
expect_equal(m2$estimate, m3$estimate, ignore_attr = TRUE)
...
if (requireNamespace("causaldata", quietly = TRUE)) {
    ...
    expect_equal(z, -2.0682935630417, tolerance = 1e-4, ignore_attr = TRUE)
}
