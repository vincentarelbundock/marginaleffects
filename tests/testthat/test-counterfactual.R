...
# size
mod <- lm(mpg ~ hp + drat + wt, mtcars)
expect_equal(nrow(datagrid(wt = 2:3, newdata = mtcars, grid_type = "counterfactual")), nrow(mtcars) * 2, ignore_attr = TRUE)
expect_equal(nrow(datagrid(wt = 2:3, newdata = mtcars)), 2, ignore_attr = TRUE)
expect_equal(nrow(datagrid(wt = 2:3, model = mod, grid_type = "counterfactual")), nrow(mtcars) * 2, ignore_attr = TRUE)
...
expect_equal(dim(res), c(128, 7), ignore_attr = TRUE)
