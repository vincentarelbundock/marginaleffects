test_that("dbarts package works", {
    skip_if_not_installed("dbarts")
    skip_if_not_installed("modeldata")

    withr_library("dbarts")
    withr_library("modeldata")
    ...
    on.exit(options(marginaleffects_posterior_center = NULL))
    ...
    expect_equal(p$estimate, mean(p1 - p0), ignore_attr = TRUE)
    ...
    expect_equal(sort(c(p0$estimate, p1$estimate)), sort(p$estimate), ignore_attr = TRUE)
