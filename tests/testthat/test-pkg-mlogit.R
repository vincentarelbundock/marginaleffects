testthat::skip_if_not_installed("mlogit")
...
dat_mlogit <- rbind(lo, hi)
dat_mlogit$term <- factor(dat_mlogit$term, levels = c("lo", "hi"))

# Test average probability predictions
p <- predictions(m, newdata = dat_mlogit, by = c("group", "term"))
...
p_custom <- predictions(m, newdata = dat_mlogit, hypothesis = h)
...
mfx <- predictions(m, newdata = dat_mlogit, hypothesis = h_mfx)
...
expect_error(avg_comparisons(m), regexp = "predictions.*supported")
expect_error(avg_slopes(m), regexp = "predictions.*supported")
