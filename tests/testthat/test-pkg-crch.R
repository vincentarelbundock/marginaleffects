testthat::skip_if_not_installed("crch")
...
mod_simple <- crch::crch(rain ~ ensmean + sqrtensmean, data = dat[1:100, ])
...
expect_slopes2(mod, type = "location", n_unique = 1)
...
expect_predictions2(model, newdata = dat, n_row = nrow(dat))
expect_predictions2(model, newdata = head(dat), n_row = 6)
