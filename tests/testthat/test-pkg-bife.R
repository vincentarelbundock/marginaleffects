testthat::skip("deprecated - don't remember why, but at some point I had reasons to doubt numerical results")
testthat::skip_if_not_installed("bife")
requiet("bife")

mod <- bife(LFP ~ AGE + I(INCH / 1000) + KID1 + KID2 + KID3 | ID, data = psid)
dat_bife <- get_modeldata(mod, additional_variables = "KID")
dat_bife <- psid <- transform(dat_bife, KID = ifelse(KID1 > 0, 1, 0))
...
pred <- predictions(mod, newdata = dat_bife, type = "response")
expect_equal(pred$estimate, predict(mod, newdata = dat_bife, type = "response"), ignore_attr = TRUE)
expect_predictions2(mod, newdata = dat_bife, type = "response", n_row = nrow(dat_bife))
...
expect_equal(s1$estimate, s2, tolerance = 1e-4, ignore_attr = TRUE)

s3 <- slopes(mod, variables = "AGE", by = "KID", newdata = psid)
s2 <- aggregate(estimate ~ KID, FUN = mean, data = s1)
expect_equal(s3$estimate, s2$estimate, tolerance = 1e-4, ignore_attr = TRUE)
...
s2 <- s2[order(s2$KID), ]
s1 <- s1[order(s1$KID), ]
expect_equal(s1$estimate, s2$s, tolerance = 1e-4, ignore_attr = TRUE)
