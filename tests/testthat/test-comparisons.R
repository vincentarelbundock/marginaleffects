...
if (requireNamespace("tibble", quietly = TRUE) && requireNamespace("dplyr", quietly = TRUE)) {
    ...
    experiment_data <- d %>%
        dplyr::sample_n(20000, weight = exp_weight, replace = TRUE) %>%
        dplyr::group_by(device, lang) %>%
        dplyr::mutate(
            treatment = sample(c('treatment', 'control'), size = dplyr::n(), replace = TRUE),
            p = dplyr::case_match(treatment, 'treatment' ~ p_treat, .default = p_control),
            y = rbinom(dplyr::n(), 1, p)
        ) %>%
        dplyr::ungroup()
