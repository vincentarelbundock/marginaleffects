...
dat_plot_pred <- transform(mtcars, am_fct = factor(am))
mod <- lm(wt ~ am_fct * mpg, data = dat_plot_pred)
...
# threenum
threenum <<- c(
    mean(dat_plot_pred$mpg) - sd(dat_plot_pred$mpg),
    mean(dat_plot_pred$mpg),
    mean(dat_plot_pred$mpg) + sd(dat_plot_pred$mpg)
)
...
dat_plot_pred2 <- data.frame(x = x, y = y)
dat_plot_pred2[["log_x"]] <- log(x)
dat_plot_pred2[["log_y"]] <- log(y)
model <- lm(log(y) ~ 1 + log(x), data = dat_plot_pred2)
...
vdiffr::expect_doppelganger("plot_predictions_points_alpha", p)


# two conditions
mod <- lm(mpg ~ hp * wt * am, data = mtcars)
p <- plot_predictions(mod, condition = c("hp", "wt"))
vdiffr::expect_doppelganger("plot_predictions_two_conditions", p)

# gray scale
mod <- lm(mpg ~ hp * wt * am, data = mtcars)
p <- plot_predictions(mod, condition = c("hp", "wt"), gray = TRUE)
vdiffr::expect_doppelganger("plot_predictions_gray_scale", p)

# continuous vs. categorical x-axis
mod <- lm(mpg ~ hp * wt * factor(cyl), mtcars)
p <- plot_predictions(mod, condition = c("cyl", "wt"))
vdiffr::expect_doppelganger("plot_predictions_categorical_xaxis", p)
p <- plot_predictions(mod, condition = c("wt", "cyl"))
vdiffr::expect_doppelganger("plot_predictions_continuous_xaxis", p)
...
vdiffr::expect_doppelganger("plot_predictions_conf99", p1)
vdiffr::expect_doppelganger("plot_predictions_conf40", p2)

p1 <- plot_predictions(mod, condition = "hp", conf.level = .99, draw = FALSE)
p2 <- data.frame(predict(mod, newdata = p1, se.fit = TRUE))
expect_equal(p1$estimate, p2$fit, tolerance = 1e-6, ignore_attr = TRUE)
expect_equal(p1$std.error, p2$se.fit, tolerance = 1e-5, ignore_attr = TRUE)
expect_equal(
    p1$conf.low,
...
vdiffr::expect_doppelganger("plot_predictions_binomial_response", p1)
vdiffr::expect_doppelganger("plot_predictions_binomial_link", p2)
...
dat_plot_pred3 <- transform(mtcars, am_fct = factor(am))
mod <- lm(wt ~ am_fct * mpg, data = dat_plot_pred3)
...
expect_equal(p1$estimate, p2$estimate, ignore_attr = TRUE)
expect_equal(p1$conf.low, p2$conf.low, ignore_attr = TRUE)
