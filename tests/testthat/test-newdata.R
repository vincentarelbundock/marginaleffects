test_that("newdata functionality works", {
    skip_if_not_installed("modelbased")
    skip_if_not_installed("emmeans")
    skip_if_not_installed("dplyr")
    skip_if_not_installed("MatchIt")

    withr_library("modelbased")
    ...
    # expect_equal(nrow(m1), nrow(m2), ignore_attr = TRUE)
    ...
    expect_equal(cmp$std.error, emm$SE, tolerance = 1e-6, ignore_attr = TRUE)
    ...
    # expect_s3_class(slopes(mod, newdata = dat, by = "cyl"), "slopes")
    ...
    cmp3 <- test()
    expect_equal(nrow(cmp1), nrow(subset(lalonde, nodegree == 1)), ignore_attr = TRUE)
    expect_equal(nrow(cmp2), nrow(subset(lalonde, nodegree == 1 & married == 1)), ignore_attr = TRUE)
    expect_equal(nrow(cmp3), nrow(subset(lalonde, nodegree == 1)), ignore_attr = TRUE)
    ...
    expect_equal(k, x, ignore_attr = TRUE)
    expect_equal(k, y, ignore_attr = TRUE)
    expect_equal(k, w, ignore_attr = TRUE)
})
