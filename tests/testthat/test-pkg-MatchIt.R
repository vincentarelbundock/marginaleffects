...
testthat::skip_if_not_installed("checkmate")
...
md <- match.data(mF)
fit1 <- lm(Y_C ~ A * (X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9), data = md, weights = weights)
pre <- avg_predictions(fit1, variables = "A", vcov = ~subclass, newdata = subset(md, A == 1), wts = "weights", by = "A")
expect_s3_class(pre, "predictions")
expect_data_frame(pre, nrows = 2)
cmp <- avg_comparisons(fit1, variables = "A", vcov = ~subclass, newdata = subset(md, A == 1), wts = "weights")
