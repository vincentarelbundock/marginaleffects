...
# Issue #1161
dat_survey <- "https://vincentarelbundock.github.io/Rdatasets/csv/AER/SmokeBan.csv"
dat_survey <- read.csv(dat_survey, na.strings = c("*", ""))
dat_survey$weights <- runif(n = nrow(dat_survey), min = 1, max = 100)
dat_survey$smoker <- factor(dat_survey$smoker)
design1 = svydesign(ids = ~1, weights = ~weights, data = dat_survey)
