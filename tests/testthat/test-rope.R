...
dat_rope <- transform(mtcars, cyl = factor(cyl))
void <- capture.output(mod <- stan_glm(mpg ~ hp + cyl, data = mtcars))
...
expect_equal(p1$p.rope.unconditional, p2, ignore_attr = TRUE)
expect_equal(p1$p.rope.unconditional, p3, ignore_attr = TRUE)
expect_equal(p1$p.rope.conditional, p4, ignore_attr = TRUE)
expect_equal(p1$p.rope.conditional, p5, ignore_attr = TRUE)
