testthat::skip_if_not_installed("mhurdle")
requiet("mhurdle")

if (packageVersion("insight") <= "1.4.0") {
    testthat::skip("Insight Issue #1114")
}

tol <- 0.001
tol_se <- 0.001

# Basic expectation tests
data("Interview", package = "mhurdle")
mod_simple <- mhurdle(shows ~ educ + size | linc,
    data = Interview, h2 = TRUE, dist = "n", method = "bhhh")
expect_slopes2(mod_simple)
expect_predictions2(mod_simple)
expect_hypotheses2(mod_simple)
expect_comparisons2(mod_simple)

data("Interview", package = "mhurdle")
m1 <- mhurdle(shows ~ 0 | linc + smsa + age + educ + size, data = Interview, h2 = TRUE, dist = "n", method = "bhhh")
m2 <- mhurdle(
    shows ~ educ + size | linc | smsa + age,
    data = Interview,
    h2 = FALSE,
    method = "bhhh",
    corr = TRUE,
    finalHessian = TRUE
)

# marginaleffects vs. margins (unit-level SEs)
set.seed(1024)
nd <- Interview[sample(seq_len(nrow(Interview)), 10), ]
mfx <- slopes(m2, newdata = nd, type = "E")
mar <- margins(m2, type = "response", data = nd, unit_ses = TRUE)

expect_equal(mfx[mfx$term == "linc", "estimate"], as.numeric(mar$dydx_linc), tolerance = tol, ignore_attr = TRUE)
expect_equal(mfx[mfx$term == "educ", "estimate"], as.numeric(mar$dydx_educ), tolerance = tol, ignore_attr = TRUE)
expect_equal(mfx[mfx$term == "age", "estimate"], as.numeric(mar$dydx_age), tolerance = tol, ignore_attr = TRUE)

expect_equal(mfx[mfx$term == "linc", "std.error"], mar$SE_dydx_linc, tolerance = tol_se, ignore_attr = TRUE)
expect_equal(mfx[mfx$term == "educ", "std.error"], mar$SE_dydx_educ, tolerance = tol_se, ignore_attr = TRUE)
expect_equal(mfx[mfx$term == "age", "std.error"], mar$SE_dydx_age, tolerance = tol_se, ignore_attr = TRUE)

# marginaleffects vs. margins: AME
mfx <- avg_slopes(m2, type = "E")
mfx <- mfx[match(c("age", "educ", "linc", "size", "smsa"), mfx$term), ]
mar <- margins(m2)
mar <- summary(mar)
expect_equal(mfx$estimate, mar$AME, tolerance = tol, ignore_attr = TRUE)
expect_equal(mfx$std.error, mar$SE, tolerance = tol_se, ignore_attr = TRUE)
