# TODO: high tolerance
testthat::skip("memory explosion")
...
set.seed(1024)
dat_brms_avg_lo <- dat_brms_avg_hi <- head(mtcars)
dat_brms_avg_hi$hp <- dat_brms_avg_hi$hp + 10
dat_brms_avg_lo$hp <- dat_brms_avg_lo$hp - 10
avg1 <- suppressWarnings(pp_average(
    m1,
    m2 = m2,
    m3 = m3,
    summary = FALSE,
    newdata = dat_brms_avg_lo
))
