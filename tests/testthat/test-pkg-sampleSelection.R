test_that("sampleSelection package works", {
    skip_if_not_installed("sampleSelection")

    withr_library("sampleSelection")
    ...
    slo1 <- avg_slopes(m, part = "selection", type = "response")
    expect_s3_class(slo1, "slopes")
    slo2 <- avg_slopes(m, part = "outcome", type = "unconditional")
