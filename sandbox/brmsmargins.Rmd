...
This section replicates some of the analyses from the `brmsmargins` vignettes to highlight the parallels and differences in syntax.

## TODO

Probably not supported by `marginaleffects`:

* `brmsmargins::marginalcoef()`

Not quite sure how to replicate:

* Contrasts for discrete predictors
* Link scale and backtransform example
...
By default, `marginaleffects` reports the median of the posterior distribution for bayesian models:

```{r}
summary(mfx)
```

We can use the `FUN` argument to compute the mean of the posterior distribution instead, which yields results that are numerically identical to those produced by `brmsmargins`:
