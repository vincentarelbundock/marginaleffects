...
M <- declare_model(
  ...
  individuals = add_level(
    N = 4, 
    U_i = rnorm(N),
    age = sample(18:70, size = N, replace = TRUE),
    gender = rbinom(N, size = 1, prob = .5),
    potential_outcomes(Y ~ Z + age + gender + U_h + U_v + U_i)
    )
)
M() |> head()
...
A = declare_assignment(
    Tcity = cluster_ra(clusters = city),
    Tperson = simple_ra()
)
...
diagnose_design(design, diagnosands = my_diagnosand)
```
````