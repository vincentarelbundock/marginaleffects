...
vignette: >
  %\VignetteIndexEntry{Bootstrap and Simulation-Based Inference}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

`marginaleffects` offers an *experimental* `inferences()` function to computate uncertainty estimates using the bootstrap and simulation-based inference.  The primary function to do this is `inferences()`. 
...
```{r}
mod |> avg_comparisons(by = "Species", variables = "Petal.Width")
```

Since this is the default method, we obtain the same results if we add the `inferences()` call in the chain:

```{r}
mod |> inferences(method = "delta") |>
       avg_comparisons(by = "Species", variables = "Petal.Width")
```


# Bootstrap

`marginaleffects` supports two bootstrap frameworks in `R`: the well-established `boot` package and the newere `rsample` package.

## `boot`

```{r}
mod |> inferences(method = "boot") |>
       avg_comparisons(by = "Species", variables = "Petal.Width")
```

All unknown arguments that we feed to `inferences()` will be pushed forward to `boot::boot()`:
...
As before, we can pass argument to `rsample::bootstraps()` through `inferences()`. For example, for stratified resampling:
...
mod |> inferences(method = "simulation") |>
       avg_comparisons(by = "Species", variables = "Petal.Width")
```
