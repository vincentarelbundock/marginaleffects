...
Note that the `slopes` function will always return estimates of zero for regressors before the vertical bar in the formula. This is because ˋmarginaleffects` increments all rows of the prediction dataset in the same way to compute slopes and contrast. Because ˋmlogit` data are in "long" format, this means that alternatives are incremenented in the same way, which does not produce alternative-specific changes in the predictors.
