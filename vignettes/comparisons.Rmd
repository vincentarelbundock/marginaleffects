...
## Conceptual: Describing the estimands
...
Notice that the contrast between 3rd and 1st is different from row to row. This reflect the fact that moving from 1st to 3rd would have a different effect on differnet 
...
> Contrast estimated for a 50 year old man and a 50 year old woman.

Note that our core scientific interest is in estimating the difference between 1st to 3rd class. We are just estimating this difference on a specific grid.

```{r}
cmp <- comparisons(mod,
  variables = list(PClass = c("1st", "3rd")),  # Step 1: Quantity
  newdata = datagrid(SexCode = 0:1, Age = 50)) # Step 2: Grid
cmp
```

We now know that moving from 1st to 3rd changes by `r sprintf("%.3f", cmp$estimate[1])` the probability of survival for 50 year old men (`SexCode=0`), and by `r sprintf("%.3f", cmp$estimate[1])` the probability of survival for 50 year old women (`SexCode=1`).


### Step 3: Averaging

We are also interested in the average change in the probability of survival when a 50 year old man and a 50 year old woman move from 3rd to 1st class.

### Hypothesis

We are also interested in testing the hypothesis that moving from 1st to 3rd has a larger effect for a 50 year old man than a 50 year old woman. This is a pairwise difference in the contrasts.

## Empirical: Estimating the estimands

```{r}
cmp <- avg_comparisons(mod,                    # Step 3: Averaging
  variables = list(PClass = c("1st", "3rd")),  # Step 1: Quantity
  newdata = datagrid(SexCode = 0:1, Age = 50)) # Step 2: Grid
cmp
```

If we compute the change in probability of survival when moving from 1st to 3rd for a 50yo man and a 50yo woman, and then take the average of those changes, we obtain: `r sprintf("%.3f", cmp$estimate)`.
...
cmp <- comparisons(mod,                    
  variables = list(PClass = c("1st", "3rd")),  # Step 1: Quantity
  newdata = datagrid(SexCode = 0:1, Age = 50), # Step 2: Grid
  hypothesis = "pairwise")                     # Step 4: Hypothesis
cmp
```

## Manual computation

Now we show how to obtain all the quantities described above using manual computation with the `predict()` function. Perhaps this will help some better understand what computations are done under the hood:
