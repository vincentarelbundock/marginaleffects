...
By default, `comparisons()` will compute estimates for every row of the original dataset that was used to fit a model. The estimation was made on `r nrow(mod$model)` observations in the titanic dataset. Therefore, if we just execute the code in the previous section, we will obtain `r nrow(mod$model)` estimates of the difference between the probability of survival in 3rd and 1st class:
...
For example, imagine that one of the control variables in your model is a variable measuring educational attainment in 4 categories: No high school, High school, Some college, Completed college. The contrast between marginal means is a weighted average of contrasts estimated in the 4 cells, and each of those contrasts will be weighted equally in the overall estimate. If the population of interest is highly unbalanced in the educational categories, then the estimate computed in this way will not be most useful.
