...
# multiple imputation
dat_mice <- mice(dat, m = 5, method = "sample", printFlag = FALSE)
dat <- complete(dat_mice, action = "all")

# fit models
mod <- lapply(dat, function(x) {
    feglm(am ~ mpg * cyl + hp,
        weight = ~gear,
        family = binomial,
        data = x)
...
lapply(seq_along(mods_feglm), \(i) 
    avg_slopes(mod[[i]], newdata = dat_completed[[i]])) |>
    mice::pool()

# slopes with weights
lapply(seq_along(mods_feglm), \(i) 
    avg_slopes(mod[[i]], newdata = dat_completed[[i]], wts = "gear")) |>
    mice::pool()
```