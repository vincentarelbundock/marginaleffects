...
`marginaleffects` support the `mice` impuation package, and any other package which can return a list of imputed data frames. 
...
Several `R` packages can impute missing data. Indeed, [the `Missing Data CRAN View`](https://cran.r-project.org/web/views/MissingData.html) lists at least a dozen alternative packages. Since user interface changes a lot from package to package, supports a single workflow which can be used, with some adaptation, with all imputation packages:

1. Use an external package to create a list of imputed data frames
2. Use the `datalist2mids()` function from the `miceadds` package to convert the list of data frames to a `mids` object.
