...
One particular problem arises in the cases of contrasts and categorical predictors. To see it, notice that when there are contrasts or categorical predictors, the `averages()` method of `marginaleffects` identifies unique estimates using two columns called `term` and `contrast`:
