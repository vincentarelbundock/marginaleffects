...
You can estimate average counterfactual marginal effects with `slopes()` by using the [`datagridcf()`](man/datagrid.html) to create a counterfactual dataset in which the full original dataset is replicated for each potential value of the `cyl` variable. Then, we tell the `by` argument to average within groups:
...
Two more functionalities deserve to be highlight. First, `fmeffects` includes functions to explore heterogeneity in marginal effects using recursive partitioning trees. The [heterogeneity vignette](vignettes/heterogeneity/) illustrates how to achieve something similar with `marginaleffects`.
