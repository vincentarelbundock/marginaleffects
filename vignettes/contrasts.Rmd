...
```{r}
avg_comparisons(mod, variables = c("cyl", "am"))
```
...
The `transform_post` argument applies a custom transformation to the average contrast.
...
We can use the `comparisons()`, `avg_comparisons()`, and `averages()` functions to obtain the same results:
...
2. Exponentiate them
3. Take the average using the `avg_comparisons()` or `averages()` functions
