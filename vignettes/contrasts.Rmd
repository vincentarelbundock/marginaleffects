...
Note: Some of the code in this vignette requires `marginaleffects` version 0.7.2 or [the development version from github.](https://vincentarelbundock.github.io/marginaleffects/#installation)
...
A big dataset of contrasts like this one can be unwieldy. A common strategy is to summarize the unit-level contrasts by taking their average. This can be achieved using the `averages()` function or the `by` argument:
...
We can also compute several average contrasts at the same time:
...
The `slopes` function automatically computes contrasts for each level of the categorical variables, relative to the baseline category (`FALSE` for logicals, and the reference level for factors), while holding all other values at their mode or mean:

```{r}
mfx <- slopes(mod) |> averages()
mfx
```
...
```{r}
comparisons(mod, variables = list(cyl = "sequential")) |> averages()
comparisons(mod, variables = list(cyl = "pairwise")) |> averages()
comparisons(mod, variables = list(cyl = "reference")) |> averages()
```
...
comparisons(mod_logit) |> averages()

comparisons(mod_logit, type = "link") |> averages()
```
...
```{r}
comparisons(mod, variables = list(hp = "iqr")) |> averages()

comparisons(mod, variables = list(hp = "sd")) |> averages()

comparisons(mod, variables = list(hp = "2sd")) |> averages()

comparisons(mod, variables = list(hp = "minmax")) |> averages()
```
...
By default, the `slopes()` and `comparisons()` functions compute marginal effects and contrasts for every row of the original dataset. These unit-level estimates can be of great interest, as discussed in [another vignette](https://vincentarelbundock.github.io/marginaleffects/articles/logistic_contrasts.html). Nevertheless, one may want to focus on one-number summaries: the `averages()` function computes the "Average Marginal Effect" or "Average Contrast," by taking the mean of all the unit-level estimates.
...
6. `transform_post`: Transform the average contrast or return them as-is.
...
The `transform_post` argument is available in the `averages()` and `averages()` functions. It applies a custom transformation to the average contrast.

The difference between `transform_post` and `transform_post` is that the former is applied *before* we take the average, and the latter is applied *to the average*. This seems like a subtle distinction, but it can be important practical implications, since a function of the average is rarely the same as the average of a function:
...
We can use the `comparisons()` and `averages()` functions to obtain the same results:
...
```{r}
comparisons(mod, transform_pre = "ratio") |> averages()
```
...
comparisons(mod, transform_pre = function(hi, lo) hi / lo) |> averages()
```

This mechanism is powerful, because it lets users create fully customized contrasts. Here is a non-sensical example:

```{r}
comparisons(mod, transform_pre = function(hi, lo) sqrt(hi) / log(lo + 10)) |> averages()
```
...
2. Exponentiate them
3. Take the average using the `averages()` function
...
comparisons(
    mod,
    transform_pre = function(hi, lo) log(hi / lo)) |>
    averages(transform_post = exp)
```

Since the `exp` function is now passed to the `transform_post` argument of `averages()` function, the exponentiation is now done only *after* unit-level contrasts have been averaged. This is what `Stata` appears to do under the hood, and the results are slightly different.
