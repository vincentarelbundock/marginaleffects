...
knitr::include_graphics("heiss_marginaleffects](https://user-images.githubusercontent.com/987057/169581274-f0d2beb6-0146-453e-91ca-a320c6ef7144.png")
```
...
We can compute other summaries of unit-level marginal effects by setting the `FUN` argument. For example, to compute "median marginal effects":
...
We can also use the `by` argument of the `summary` or `tidy` functions to compute the average marginal effects *within different subgroups* of the observed data, based on values of the regressors. For example, to compute the average marginal effects of Bill Length for each Species, we do:
