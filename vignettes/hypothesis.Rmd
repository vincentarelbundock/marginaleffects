---
title: "Hypothesis Tests, Delta Method, and Custom Contrasts"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Hypothesis Tests, Delta Method, and Custom Contrasts}
...
Version 0.6.0 and [the development version of `marginaleffects`](https://vincentarelbundock.github.io/marginaleffects/#getting-started) include a very flexible function called `deltamethod()` which can be used to compute estimates and standard errors of functions of model parameters. `deltamethod()` emulates the behavior of the excellent and well-established `car::deltaMethod` and `car::linearHypothesis` functions, but it supports more models, requires fewer dependencies, and offers some convenience features like shortcuts for robust standard errors.
...
When `FUN` and `hypothesis` are `NULL`, `deltamethod()` returns a data.frame of raw estimates:
...
We can use shortcuts like `r1`, `r2`, `...` to identify the position of each parameter in the output of `FUN`. For example, `r2=r3` is equivalent to `hp=wt` because those term names appear in the 2nd and 3rd row when we call `deltamethod(mod)`. 
...
The `FUN` argument can be used to compute standard errors for arbitrary functions of model parameters. This user-supplied function must accept a single model object, and return a numeric vector or a data.frame with two columns named `term` and `value`.
...
Test of equality between two predictions (row 2 vs row 1):

```{r}
f <- function(x) predict(x, newdata = mtcars)
deltamethod(mod, FUN = f, hypothesis = "r2 = r3")
```
