...
```{r, include = FALSE}
options(width = 120)
hyp <- hypotheses(p, region = c(17, 18))
```

Now we specify an equivalence "region" for predictions between 17 and 18:

```{r}
hypotheses(p, region = c(17, 18))
```

The results above allow us to draw three conclusions:
...
The same syntax can be used to conduct tests for all the quantities produced by the `marginaleffects` package. For example, imagine that, for substantive or theoretical reasons, an average slope between -0.1 and 0 is uninteresting. We can conduct an equivalence test to check if this is the case:
...
pval <- slopes(
    ...
    variables = "hp",
    by = TRUE,
    region = c(-.1, .0))$p.value.equ
```

The p value is `r sprintf("%.4f", pval)`, which suggests that we can reject the hypothesis that the parameter falls outside the equivalence region (or "substantive irrelevance region"). 
...
```{r}
hypotheses(mod, region = c(5, 7))[4, ]
```

```{r, include = FALSE}
hyp <- hypotheses(mod, region = c(5, 7))[4, "p.value.equ"]
```
...
#### Example 3: `comparisons()` with the `hypothesis` argument

Now consider contrasts at the median between levels of the `gear` variable:
...
Now, we test if this difference in differences falls in the a region of practical significance (or "equivalence region") between 0 and 10:
...
The p value gives us reason to believe that the quantity of interest does not fall outside the specified region.

#### Example 4: Marginal means and `emmeans`
