...
The `vcov` argument behaves in the same was as in the `marginaleffects()` function. Itallows us to easily compute robust standard errors:
...
The `FUN` argument can be used to compute standard errors for arbitrary functions of model parameters. This function accepts a model object and returns a numeric vector or a data.frame with two columns named `term` and `value`.
...
```{r}
f <- function(x) predict(x, type = "link", newdata = mtcars)
deltamethod(mod, FUN = f, hypothesis = "r1 = r2")
```
