...
1. The p value for the non-superiority test is `r sprintf("%.4f", hyp$p.value.sup)`. This suggests that we *cannot* reject the hypothesis that the parameter (`r sprintf("%.4f", hyp$estimate)`) is above the [17,18] interval.
2. The p value for the non-inferiority test is `r sprintf("%.4f", hyp$p.value.inf)`. This suggests that we *can* reject the hypothesis that the parameter is below the [17,18] interval.
3. The p value for the equivalence test is `r sprintf("%.4f", hyp$p.value.equ)`. This suggests that we *cannot* reject the hypothesis that the parameter falls outside the equivalence region.
...
```{r, include = FALSE}
hyp <- hypotheses(mod, equivalence = c(5, 7))[4, "p.value.equ"]
```
...
```
```{r, include = FALSE}
hyp <- avg_slopes(mod, variables = "hp", equivalence = c(-.1, .1))$p.value.equ
```
...
#### Example 4: Difference between slopes
...
The average slope -- partial derivative of `mpg` with respect to `hp` -- differs based on the value of `gear`:

```{r}
avg_slopes(int, variables = "hp", by = "gear")
```

Are these slopes different from one another? Let's look at the pairwise comparisons between them:

```{r}
avg_slopes(int, variables = "hp", by = "gear", hypothesis = "pairwise")
```
...
The `p (Sup)` column shows that the difference between the average slope when `gear` is 3 and `gear` is 5 can be said to be equivalent to the specified interval. However, there are good reasons to think that the other two pairwise comparisons may fall outside the interval.


#### Example 5: `comparisons()` with the `hypothesis` argument

Now consider average contrast between levels of the `gear` variable:

```{r}
avg_comparisons(mod, variables = "gear")
```

We can compare those contrasts using the `hypothesis` argument, giving us a sort of "difference in differences":

```{r}
avg_comparisons(mod, variables = "gear", hypothesis = "pairwise")
```

Now, we test if this difference in differences falls in the a region of practical significance (or "equivalence region") between -8 and 8:

```{r}
avg_comparisons(mod, variables = "gear", hypothesis = "pairwise") |>
    hypotheses(equivalence = c(-8, 8))
```

The p value gives us reason to believe that the quantity of interest does not fall outside the specified interval.

#### Example 6: Marginal means and `emmeans`

This final example shows the equivalence between results produced by the `emmeans` package and the `marginalmeans()` function:
