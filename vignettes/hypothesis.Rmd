...
This vignette introduces the `deltamethod()` function, and the `hypothesis` argument of the `comparisons()`, `slopes()`, and `predictions()` function. These features allow users to conduct linear and non-linear hypothesis tests and to compute custom contrasts (linear combinations) between parameters.
...
Is the marginal effect of `am` different from the marginal effect of `vs`? To answer this question we can run a linear hypothesis test using the `deltamethod` function:

```{r}
deltamethod(mfx, hypothesis = "am = vs")
```
...
```{r}
deltamethod(p, hypothesis = "b1 = b2")
```
