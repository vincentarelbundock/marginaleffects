...
By default, predict produces predictions on the "response" scale, so the adjusted predictions should be interpreted on that scale. However, users can pass a string to the type argument, and predictions will consider different outcomes.
...
The "reference grid" may or may not correspond to actual observations in the dataset used to fit the model; the example values given above could match the mean values of each variable, or they could represent a specific observed (or hypothetical) individual. 

The following sections will introduce to different ways to select a reference grid over which we want to compute adjusted predictions.
...
<!-- Revoir le titre du tableau pour mettre cyl -->
<!-- Mettre KableExtra faciliterait probablement la compréhension de marginaleffects -->
...
predictions(
  ...
  tt(caption = "A table of Adjusted Predictions", theme = "bootstrap", digits = 4, width = 0.8) |>
  group_tt(j = list("cyl" = 3)) |>
  style_tt(bootstrap_css_rule = "caption{
    font-weight: bold
  }")
```


#### **2. `variables`: Counterfactual predictions**
...
#### **3. `counterfactual` data grid <!-- Pas certain de comment l'inclure dans la section -->**
...
<!-- Ajouter paragraphes avec vcov et conf_level ? ou 'get started' est assez -->
...
<!-- Ajouter éléments de l'article de la section test à propos de la fonction hypothesis() ? ou 'get started' est assez-->
