...
If you try to apply the `plot()` function directly to a `marginaleffects` object, you will get an error like this one:
...
The reason for this error is that the query is underspecified. `marginaleffects` allows users to draw many different kinds of plots, for many different quantities of interest. A simple `plot()` call does not give sufficient information to know exactly what plot the user needs: marginal or conditional, which subgroups, for what contrasts, etc. Allowing these options would require duplicating many of the arguments already implemented in the `plot_predictions()`, `plot_comparisons()`, and `plot_slopes()` functions. To be more explicit and to avoid the confusion that would arise from two parallel user interface, the `plot()` function raises an error and points to an appropriate function to use.