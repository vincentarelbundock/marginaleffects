...
This vignette shows how to add support for new models and add new functionality for supported models. If you add support for a new model object, please consider submitting your code for inclusion in the package.
...
1. `set_coef()`
    - Mandatory arguments: `model`, `coefs` (named vector), `...`
...
1. `get_predict()`
    ...
    - Returns: A data frame with two columns: a unique `rowid` and a column of `predicted` values.
...
Our new `get_predict.customclass` method takes this matrix of predictions, modifies it, and reshapes it to return a data frame with three columns: `rowid`, `group`, and `predicted`:
...
Finally, we can call any `marginaleffects` function and obtain results. Notice that our object of class `customclass` now produces different results than the default `mblogit` object:

```{r}
predictions(model) |> summary()

predictions(model_custom) |> summary()
```