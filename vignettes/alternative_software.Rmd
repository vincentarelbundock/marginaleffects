...
The standard errors produced by `Stata` for counterfactual marginal effects use a "Jacobian at the mean." 

The Jacobian is stored in an attribute of the `marginaleffects` object. This Jacobian has a number of rows equal to the dataset, multiplied by the number of unique values in the variable of interest (remember that we replicated the whole dataset several times):
...
To compute the same standard errors as `Stata`, we use the mean Jacobian:
