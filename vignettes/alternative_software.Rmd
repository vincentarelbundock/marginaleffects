...
If you do not like `marginaleffects`, you may want to consider one of the free alternatives described below (or `Stata`, if you must).

# `Stata`

`Stata` is a good but very expensive software package for statistical analysis. This section compares `Stata`'s `margins` command to the `marginaleffects`.
...
The `margins` and `predictions` packages for `R` were designed by Thomas Leeper to emulate the behavior of the `margins` command from `Stata`. These packages are trailblazers and strongly influenced the development of `marginaleffects`. The main benefits of `marginaleffects` over these packages are:
