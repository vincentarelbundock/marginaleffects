...
Step 1 is to fit a regression model of the outcome on the treatment a control variables:
...
Steps 2 and 3 require us to replicate the full dataset by setting the `qsmk` treatment to counterfactual values. We can accomplish this using the `datagrid()` function and its `grid_type="counterfactual"` argument. This function can be used in combination any of the functions from the `marginaleffects` package. 
...
Now we want to compute the treatment effect, as the difference in average predicted outcomes in the two counterfactual datasets. To do This is equivalent to applying the parametric g-formula. There are three important things to note in the command that follows:
...
* The `datagrid()` function is used with the `grid_type="counterfactual"` argument. This replicates the entire dataset once for every combination of the specified regressors. In this case, we specified `qsmk = c(0, 1)`, so this will replicate the dataset twice.
* `comparisons()` automatically produces estimates of uncertainty.

```{r}
cmp <- comparisons(
    std,
    variables = list(qsmk = 0:1),
    newdata = datagrid(qsmk = 0:1, grid_type = "counterfactual"))
summary(cmp)
```
...
Although manual computation is simple, it does not provide uncertainty estimates. In contrast, `comparisons()` has already computed the standard error and confidence intervals using the delta method.
...
The results are close to those that we obtained with `comparisons()`, although the confidence intervals differ slightly because of the difference between bootstraping and the delta method.
