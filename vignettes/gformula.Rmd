...
Steps 2 and 3 require us to replicate the full dataset by setting the `qsmk` treatment to counterfactual values. We can accomplish this using the `datagrid()` function and its `grid_type="counterfactual"` argument. This function can be used in combination with any of the functions from the `marginaleffects` package. 
