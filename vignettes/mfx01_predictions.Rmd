...
Noticed that we had to use the `inherit.aes = FALSE` argument in `geom_point()`. This is because we want to avoid conflicts between the `mtcars` data set from which we want to draw points, and the other dataset that `plot_cap()` uses internally to draw the original plot.
