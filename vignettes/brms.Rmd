...
Use `slopes()` to [compute marginal effects (slopes of the regression equation)](https://vincentarelbundock.github.io/marginaleffects/articles/slopes.html) for each row of the dataset, and use `)` to compute "Average Marginal Effects", that is, the average of all observation-level marginal effects: 
...
We can average/marginalize/integrate across random effects with the `averages` function or the `by` argument:

```{r}
predictions(mod, newdata = datagrid(x = 0, firm = unique)) |> averages()

predictions(mod, newdata = datagrid(x = 0:1, firm = unique), by = "x")
```

We can also draw from the (assumed gaussian) population distribution of random effects, by asking `predictions()` to make predictions for new "levels" of the random effects. If we then take an average of predictions using `averages()` or `by`, we will have ["integrated out the random effects", as described in the `brmsmargins` package vignette.](https://joshuawiley.com/brmsmargins/articles/mixed-effects-slopes.html#integrating-out-random-effects) In the code below, we make predictions for 100 firm identifiers which were not in the original dataset. We also ask `predictions()` to push forward the `allow_new_levels` and `sample_new_levels` arguments to the `brms::posterior_epred` function:
...
See the [alternative software vignette](https://vincentarelbundock.github.io/marginaleffects/articles/alternative_software.html) for more information on `brmsmargins`.
...
```{r}
comparisons(mod, variables = list(lifeExp = "sd")) |> averages()
```
...
slopes(mod, type = "link") |> averages()

slopes(mod, dpar = "hu") |> averages()

slopes(mod, dpar = "hu", type = "link") |> averages()
```
...
Note: The rest of this section requires version 0.8.1 of `marginaleffects`, or [the development version.](https://vincentarelbundock.github.io/marginaleffects/#installation)
...
Notice that we can compute average contrasts in two different ways, using the `averages()` function or the `transform_pre` argument:
