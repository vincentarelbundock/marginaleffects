...
```{r}
plot_slopes(mod, effect = "woman", condition = "age")
```
...
Notice that we can compute average contrasts in two different ways, using the `avg_comparisons()` function or the `transform_pre` argument:
