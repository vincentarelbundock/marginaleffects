...
Use `slopes()` to [compute marginal effects (slopes of the regression equation)](https://vincentarelbundock.github.io/marginaleffects/articles/marginaleffects.html) for each row of the dataset, and use `summary()` to compute "Average Marginal Effects", that is, the average of all observation-level marginal effects: 
...
slopes(mod, type = "link") |> summary()

slopes(mod, dpar = "hu") |> summary()

slopes(mod, dpar = "hu", type = "link") |> summary()
```
