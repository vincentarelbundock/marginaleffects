...
We can average/marginalize/integrate across random effects with the `tidy` function or the `by` argument:

```{r}
predictions(mod, newdata = datagrid(x = 0, firm = unique)) |> tidy()

predictions(mod, newdata = datagrid(x = 0:1, firm = unique), by = "x")
```

We can also draw from the (assumed gaussian) population distribution of random effects, by asking `predictions()` to make predictions for new "levels" of the random effects. If we then take an average of predictions using `tidy()` or `by`, we will have ["integrated out the random effects", as described in the `brmsmargins` package vignette.](https://joshuawiley.com/brmsmargins/articles/mixed-effects-marginaleffects.html#integrating-out-random-effects) In the code below, we make predictions for 100 firm identifiers which were not in the original dataset. We also ask `predictions()` to push forward the `allow_new_levels` and `sample_new_levels` arguments to the `brms::posterior_epred` function:
...
We can "integrate out" random effects in the other `marginaleffects` functions too. For instance,
...
Notice that we can compute average contrasts in two different ways, using the `tidy()` function or the `transform_pre` argument:
