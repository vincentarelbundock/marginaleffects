...
We can also use the `group` argument compute the average marginal effects for subgroups of the observed data, based on values of the regressors. For example, to compute the average marginal effects of `flipper_length_mm` for each value of `species`, we do:

```{r}
summary(mfx, group = "species")
```
