...
This vignette explains how to create custom contrasts and slopes, and how to transform predictions, contrasts, and slopes using arbitrary functions.
...
The `transform_post` argument is available in the `averages()` function. It applies a custom transformation to the average contrast.
...
Since the `exp` function is now passed to the `transform_post` argument of `averages()` function, the exponentiation is now done only *after* unit-level contrasts have been averaged. This is what `Stata` appears to does under the hood, and the results are slightly different.
...
For **predictions**, we need to exponentiate the results to scale them back up to dollar amounts. We can do this by post-processing the results (e.g. with `dplyr::mutate(estimate = exp(estimate))`), or we can use the `transform_post` argument in `predictions()` to pass the results to `exp()` after getting calculated:
...
slopes_data <- comparisons(
  ...
  select(lifeExp, comparison = estimate) |>
  left_join(predictions_data, by = "lifeExp") |>
  # Point-slope formula: (y - y1) = m(x - x1)
  mutate(intercept = comparison * (-lifeExp) + estimate)

ggplot(predictions_data, aes(x = lifeExp, y = estimate)) +
  geom_line(size = 1) + 
