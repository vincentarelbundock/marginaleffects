sanitize_hypothesis_formula <- function(hypothesis) {
    ...
    if (!length(hypothesis)[1] %in% 0:1) {
        stop(
            "The left-hand side of the `hypothesis` formula must be of length 0 or 1.",
            call. = FALSE
        )
    }
    ...
    if (!length(hypothesis)[2] %in% 1:2) {
        stop(
            "The right-hand side of the `hypothesis` formula must have 1 or two parts, separated by |",
            call. = FALSE
        )
    }
    ...
    if ("x" %in% rhs) {
        ...
        if (length(asis) == 1) {
            if (length(lhs) != 0) {
                stop(
                    "The left-hand side of `hypothesis` must be empty when using custom functions.",
                    call. = FALSE
                )
            }
            asis <- sub("^I\\((.*)\\)$", "\\1", asis)
            ...
            stop(
                "Only one custom function is supported at a time on the right-hand side of the `hypothesis` formula.",
                call. = FALSE
            )
        }
    }
    ...
    checkmate::assert_choice(
        rhs,
        valid,
        .var.name = "Right-hand side of `hypothesis` formula"
    )
