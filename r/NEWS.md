...
* Automatic differentiation now lowers recorded prediction/comparison plans to a single JAX pipeline and uses the normal R result pipeline for estimates, labels, and row ordering. This removes duplicated group/order reconstruction and adds autodiff support for `wts`, `by` data frames, matrix/vector hypotheses, and `comparison = "ratio"` with grouped averages.
