...
* New `jacobian_method` component records which differentiation path produced the Jacobian: `components(x, "jacobian_method")` returns `"analytic"`, `"autodiff"`, `"numeric"`, or `"custom"`. The label reflects what actually ran: a custom Jacobian function which returns `NULL` falls back to numerical differentiation and is labeled `"numeric"`.
* `autodiff(TRUE)` now takes priority over the analytic Jacobian, as documented: enabling automatic differentiation is an explicit per-session opt-in, so the analytic path no longer intercepts estimands the JAX pipeline can lower. When autodiff is disabled or declines an estimand, the order remains analytic first, then numeric.
